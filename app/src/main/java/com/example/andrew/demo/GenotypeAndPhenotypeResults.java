...
public class GenotypeAndPhenotypeResults extends AppCompatActivity {

    public static final String TAG = GenotypeAndPhenotypeResults.class.getName();
    ZoomRecyclerView zlvChart;
    ArrayList<String> listOfData = new ArrayList<>();
    RecyclerViewAdapterGenotypeAndPhenotypeResults recyclerViewAdapter;
...
//    ArrayList<String> suggestions = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_genotype_and_phenotype_results);
        ...
        tvSummary.setText("Patient PGxOnePlus Genotype and Phenotype Results for Smith, John");
        ...
        recyclerViewAdapter = new RecyclerViewAdapterGenotypeAndPhenotypeResults(listOfData);
        if (zlvChart != null) {
            zlvChart.setAdapter(recyclerViewAdapter);
        }
        if (getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT) {
            zlvChart.setMaxHeight(508);
        } else {
            zlvChart.setMaxHeight(244);
        }
        ...
        View header = layoutInflater.inflate(R.layout.genotype_phenotype_header, zlvChart, false);
...
//                GenotypeAndPhenotypeResults.this,
...
//        });
        ...
        searchView.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
            ...
            public boolean onQueryTextSubmit(String newText) { //when the user presses the enter button
                ...
                if (!newText.isEmpty()) {
                    for (String data : listOfData) {
                        ...
                        for (int i = 0; i < data.length(); i++) {
                            if (columnNum == 9) { //only 8 columns displayed, so breaks out of loop at 9
                                break;
                            } else if (data.charAt(i) == '\t'){
                                columnNum++;
                                sb.append(' ');
                            } else if (columnNum > 5) { //first five columns not included in search
                    ...
                    recyclerViewAdapter = new RecyclerViewAdapterGenotypeAndPhenotypeResults(newList);
                    zlvChart.setAdapter(recyclerViewAdapter);
                ...
                //TODO: Temporarily closes when submitted and "G6PD" is typed in

...
public class PortablePatientTable extends AppCompatActivity {

    public static final String TAG = PortablePatientTable.class.getName();
    ZoomRecyclerView zlvChart;
    ArrayList<String> listOfData = new ArrayList<>();
    RecyclerViewAdapterPortablePatientTable recyclerViewAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_portable_patient_table);
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                if (!line.startsWith("ZNA") //does not include drug interactions and certain other data
                        && !line.startsWith("DDI") //drug drug
                        && !line.startsWith("DFI") //drug food
                        && !line.startsWith("DAI") //drug alcohol
                        && !line.startsWith("DLI") //drug lifestyle
                        && !line.startsWith("\t")) {
        ...
        tvSummary.setText("Portable Patient PGxOnePlus Genotype Results and Drug Information by Specialty for");
        ...
        recyclerViewAdapter = new RecyclerViewAdapterPortablePatientTable(listOfData);
        if (zlvChart != null) {
            zlvChart.setAdapter(recyclerViewAdapter);
        }
        if (getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT) {
            zlvChart.setMaxHeight(489);
        } else {
            zlvChart.setMaxHeight(230);
        }
        ...
        View header = layoutInflater.inflate(R.layout.header_portable_patient_table, zlvChart, false);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        searchView.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
            ...
            public boolean onQueryTextSubmit(String newText) { //when the user presses the enter button
                ...
                //TODO: Add this feature to other activities, changing else if (columnNum > 2) to take out first column
                StringBuilder sb = new StringBuilder();
                int columnNum;
                if (!newText.isEmpty()) {
                    for (String data : listOfData) {
                        ...
                        for (int i = 0; i < data.length(); i++) {
                            if (columnNum == 9) {
                                ...
                                columnNum++;
                                sb.append(' ');
                            } else if (columnNum != 2) { //words that represent images are not included in search
                    ...
                    recyclerViewAdapter = new RecyclerViewAdapterPortablePatientTable(newList);
                    zlvChart.setAdapter(recyclerViewAdapter);
